...
func TestSceneEnterShopJoinsLiveGroup(t *testing.T) {
	setting.Default()
	Manager = NewClientManager()
	client := NewClient("scene-client", "10001", &websocket.Conn{})
	client.UserId = "42"
	...
	payload, _ := json.Marshal(map[string]string{
		...
		"scene":   "shop",
		"room_id": "888",
		"extend":  `{"role":0,"nickName":"tester"}`,
	})
	client.handleTextMessage(payload)

	groupName := livecode.GroupName("888")
	Convey("shop scene.enter joins live group", t, func() {
		So(SceneChannel("shop", "888"), ShouldEqual, groupName)
		So(client.RoomId, ShouldEqual, "888")
		So(len(Manager.GetGroupClientList(util.GenGroupKey("10001", groupName))), ShouldEqual, 1)
	})
}

func TestSceneAckForInvalidRequest(t *testing.T) {
	setting.Default()
	Manager = NewClientManager()
	ToClientChan = make(chan clientInfo, 10)
	client := NewClient("scene-ack-client", "10001", &websocket.Conn{})
	client.UserId = "42"
	Manager.AddClient(client)
	...
	Convey("invalid scene enter request gets ack", t, func() {
		So(normalizeSceneAction("join", ""), ShouldEqual, "scene.enter")
		select {
		case item := <-ToClientChan:
			So(item.MessageId, ShouldEqual, "req-1")
			So(item.Code, ShouldEqual, retcode.ROOM_ID_ERROR)
		default:
			t.Fatal("expected scene ack")
		}
	})
}
